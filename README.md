.env_EXAMPLE локально переименовать в .env и положить туда свой токен. 
...
2. Удалять из бд неактивное время для мастеров
3. Добавить валидацию с помощью pydantic
4. Понять куда деть функционал для админов и мастеров. Понятно, что делать их в одном боте не очень
5. Переделать UI под наши цели и определиться с основными концепциями
6. Добавить обработку, что бот заблочен
7. Переделать запись к мастеру, так чтобы вводился id нужного мастера и сразу переходить к профилю мастера
8. В профиле можно добавить в избранное, оставить отзыв, посмотреть список услуг, кнопка перейти к полной анкете на сайт. Висит пара фоток, основная инфа. Инфа про оплату, связаться с мастером, пожаловаться на анкету.
9. Мои записи: оставить чаевые, перейти в профиль мастера, начать спор.
10. Мои записи: перейти в профиль, отменить, связаться. Подумать про механзим подтверждений (типа если можно без предоплаты) (Время отмены без платы настраивает мастер)
...
6. **Выйти**
   ----Другой бот-------
7. **Для мастеров** (доступ только для мастеров, скрыто для обычных пользователей)
8. **Для админов** 
